...
//  assert("Parsing a variable...");
  Token* currentIdent = lookAhead;
  int identEndCol = currentIdent->colNo + strlen(currentIdent->string) - 1;
  
  eat(TK_IDENT);
   if (lookAhead->colNo > identEndCol + 2) {
    missingToken(SB_COLON, lookAhead->lineNo, identEndCol + 2);
  }
  if (lookAhead->tokenType != SB_COLON) {
    missingToken(SB_COLON, lookAhead->lineNo, lookAhead->colNo);
  }
  eat(SB_COLON);
  compileType();
  eat(SB_SEMICOLON);
